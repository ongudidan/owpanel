...
force_reset_mysql_root_password() {
    ...
    # For MariaDB 10.4+, mysql.user is a view. modifying it directly via UPDATE is dangerous/fails.
    # We use ALTER USER which works reliably in modern versions.
    ...
    # Ensure raw mysqld execution effectively
...
if [ -n "$REPO_DIR" ] && [ -f "$REPO_DIR/resources/extra/setup_missing_ssl_file.sh" ]; then
    bash "$REPO_DIR/resources/extra/setup_missing_ssl_file.sh"
# else
...
if olspanel reset_admin_password "$WEB_ADMIN_PASS"; then
    ...
    /root/venv/bin/python /usr/local/lsws/Example/html/mypanel/manage.py reset_admin_password "$WEB_ADMIN_PASS"
fi
